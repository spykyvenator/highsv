...
TPrint *tprint_create (GOutputStream *ostr, gboolean borders, gboolean show_header, gint spaces_left, gint spaces_between)
{
    TPrint *tprint;
    ...
    tprint->fmt_double = g_strdup ("%0.5f");
...
static void tprint_print_with_borders (TPrint *tprint)
{
    ...
    if (tprint->show_header) {
        ...
        pToF (tprint->ostr, "%*s", tprint->spaces_left, "");
        for (l = g_list_first (tprint->l_columns); l; l = g_list_next (l)) {
            TPrintColumn *col = (TPrintColumn *) l->data;
